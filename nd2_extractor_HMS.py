...
print("Do you want to predict the number of timepoints automatically?")
answer = input()
if answer == "n":
...
with ND2Reader_SDK(directory) as frames:
    for FOV in tqdm(FOVs_list, desc = "Overall (FOV) progress"):
        for channel in tqdm(CHANNELS_list, desc = "Channel progress in FOV {}".format(FOV), leave = False):
            ...
            Parallel(prefer="threads", n_jobs = 500)(delayed(save_image)(frames[i], i, img_format) for i in tqdm(range(num_t), desc = "Frame progress in channel {}".format(IMG_CHANNELS[channel]), leave = False) )
